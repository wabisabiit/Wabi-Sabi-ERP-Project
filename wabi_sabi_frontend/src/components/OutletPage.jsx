...
/* Small inline loading UI (spinner + text) */
function OutletLoadingInline() {
  return (
...
/* ========= Location multi-select (similar to Emp/Contact) ========= */
function OutletLocationSelect({ value = [], onChange, options = [] }) {
  ...
  return (
    <div className="out-loc" ref={wrapRef}>
      <button
        type="button"
        className="out-loc-pill"
        onClick={() => setOpen((v) => !v)}
      >
        <span className="out-loc-text">Select Location</span>
        <span className="out-loc-badge">{value.length}</span>
        <span className="material-icons out-loc-caret">expand_more</span>
      </button>

      {open && (
        <div className="out-loc-pop">
          <div className="out-loc-search">
            <input
              value={q}
          ...
          <div className="out-loc-list">
            {filtered.length === 0 ? (
              <div className="out-loc-empty">No results</div>
            ) : (
              filtered.map((opt) => {
                const checked = value.includes(opt);
                return (
                  <label key={opt} className="out-loc-item" title={opt}>
                    <input
                      type="checkbox"
                      checked={checked}
                      onChange={() => toggle(opt)}
                    />
                    <span className="out-loc-txt">{opt}</span>
                  </label>
                );
              })
            )}
...
export default function OutletPage() {
  ...
  // NEW: filter strip toggle + selected locations
  ...
  // options for Select Location = unique outlet names
  const LOCATION_OPTIONS = useMemo(
    () =>
      Array.from(new Set(outlets.map((o) => o.name).filter((x) => !!x))),
    [outlets]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      out = out.filter((r) =>
        [r.type, r.name, r.displayName, r.contactNo, r.openingDate, r.month].some(
    ...
    if (locations.length > 0) {
      ...
      out = out.filter((r) =>
        locSet.has(String(r.name || "").toLowerCase())
      );
    }

    return out;
  }, [query, outlets, locations]);
  ...
  const exportExcel = () => {
    ...
    const th = headers
      .map(
        (h) =>
          `<th style="font-weight:600;border:1px solid #d9dee6;text-align:left;padding:6px 8px;background:#f5f7fb;">${h}</th>`
      )
      .join("");
    const tr = rows
      .map(
        (r) =>
          `<tr>${r
            .map(
              (c) =>
                `<td style=\"border:1px solid #d9dee6;padding:6px 8px;\">${String(
                  c ?? ""
                )}</td>`
            )
            .join("")}</tr>`
      ...
      <div className="out-card">
        ...
        <div className="out-toolbar">
          ...
          <div className="out-export" ref={exportRef}>
            ...
            {exportOpen && (
              <div className="out-export-pop" role="menu">
                <button className="out-export-item" onClick={exportExcel}>
                  Excel
                </button>
                <button className="out-export-item" onClick={exportPDF}>
                  PDF
                </button>
              </div>
            )}
          ...
          <button
            ...
            onClick={() => navigate("/admin/outlet/new")}
          >
            Create New
          </button>
        </div>

        {/* Filter strip – Select Location */}
        {openFilter && (
          <div className="out-filterstrip">
            <div className="out-field">
              <div className="out-field-label">Select Location</div>
              <OutletLocationSelect
                value={locations}
          ...
          <table className="out-table">
            <thead>
              <tr>
                ...
                <th className="col-type th-sort">
                  <span>Outlet Type</span>
                </th>
                <th className="col-name th-sort">
                  <span>Name</span>
                </th>
                <th className="col-cname th-sort">
                  <span>Display Name</span>
                </th>
                <th className="col-cno th-sort">
                  <span>Contact No</span>
                </th>
                <th className="col-year th-sort">
                  <span>Date of Opening</span>
                </th>
                <th className="col-month th-sort">
                  <span>Month Interval</span>
                </th>
                <th className="col-actions th-sort">
                  <span>Actions</span>
                </th>
              </tr>
            </thead>
