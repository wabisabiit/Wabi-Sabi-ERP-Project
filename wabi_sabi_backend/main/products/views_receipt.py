...
def _parse_ymd_or_ddmmyyyy(v: str):
    v = (v or "").strip()
    if not v:
        return None
    d = parse_date(v)
...
def _user_outlet_location(request):
...
class _StdReportPagination(PageNumberPagination):
    page_size = 10
    page_size_query_param = "page_size"
    max_page_size = 500


class SalesmanListForReport(APIView):
    """
    ✅ Dropdown list for Salesman Name (admin: all salesmen, manager: only their outlet salesmen)
    GET /api/reports/salesman-report/salesmen/
    """
    permission_classes = [permissions.IsAuthenticated]

    def get(self, request):
        user = getattr(request, "user", None)
        qs = Employee.objects.select_related("outlet", "outlet__location", "user").filter(is_active=True)

        # Only STAFF are salesmen (safe default)
        qs = qs.filter(role="STAFF")

        # Manager scoping
        loc = _user_outlet_location(request)
        if loc and not (user and user.is_superuser):
            qs = qs.filter(outlet__location_id=loc.id)

        rows = []
        for e in qs.order_by("name", "id")[:2000]:
            phone = (getattr(e, "phone", "") or "").strip()
            nm = (getattr(e, "name", "") or getattr(getattr(e, "user", None), "username", "") or "").strip()
            label = nm
            if phone:
                label = f"{nm} - {phone}"
            rows.append({
                ...
                "phone": phone,
                "label": label,
                "location": (getattr(getattr(getattr(e, "outlet", None), "location", None), "name", "") or ""),
                "location_code": (getattr(getattr(getattr(e, "outlet", None), "location", None), "code", "") or ""),
            })

        return Response({"results": rows}, status=status.HTTP_200_OK)


class SalesmanSalesReport(APIView):
    """
    ✅ Salesman Report (line-wise)
    GET /api/reports/salesman-report/
      - date_from, date_to
      - location (multi): ?location=UV&location=TN   (code OR name)
      - salesman_id
      - q (invoice search)
      - page/page_size or all=1

    Columns:
      - salesman name
      - customer name = outlet name (salesman's outlet location)
      - invoice no
      - date
      - product name
      - qty (default 1)
      - amount (line amount = sp*qty)
      - taxable amount:
          if sp > 2500 => sp - sp*18/100
          else         => sp - sp*5/100
          (× qty)
      - created_by = outlet name (same as above)
    """
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get(self, request):
        ...
        df = _parse_ymd_or_ddmmyyyy(request.GET.get("date_from", ""))
        dt = _parse_ymd_or_ddmmyyyy(request.GET.get("date_to", ""))

        # defaults: wide range if missing
        if not df:
            df = date(2000, 1, 1)
        if not dt:
            dt = date(2100, 1, 1)
        ...
        salesman_id = (request.GET.get("salesman_id") or "").strip()
        q = (request.GET.get("q") or "").strip()

        # Manager scoping
        forced_loc = _user_outlet_location(request)
        if forced_loc and not (user and user.is_superuser):
            locs = [forced_loc.code] if forced_loc.code else [forced_loc.name]
        ...
        qs = (
            ...
            .prefetch_related(
                Prefetch("sale__payments", queryset=SalePayment.objects.all())
            )
            .filter(
        ...
        # location filter (code OR name)
        if locs and "All" not in locs:
            loc_q = Q(sale__salesman__outlet__location__code__in=locs) | Q(
                sale__salesman__outlet__location__name__in=locs
            )
            qs = qs.filter(loc_q)
        ...
        qs = qs.order_by("-sale__transaction_date", "-sale_id", "-id")
        ...
        if want_all:
            ...
            paginator = _StdReportPagination()
            page_obj = paginator.paginate_queryset(qs, request)
            items = page_obj
            page = int(request.GET.get("page") or 1)
            page_size = int(request.GET.get("page_size") or paginator.page_size)
        ...
        sr_start = 1 if want_all else ((max(page, 1) - 1) * max(page_size, 1) + 1)

        for i, ln in enumerate(items, start=sr_start):
            sale = getattr(ln, "sale", None)
            emp = getattr(sale, "salesman", None)

            loc_obj = getattr(getattr(getattr(emp, "outlet", None), "location", None), "name", "") or ""
            salesman_name = (getattr(emp, "name", "") or "").strip()
            ...
            sp = Decimal(getattr(ln, "sp", 0) or 0)

            # Amount = line sp * qty
            line_amt = q2(sp * Decimal(qty))
            total_sales += line_amt

            # Taxable as per your rule (line-wise)
            rate = Decimal("0.18") if sp > Decimal("2500") else Decimal("0.05")
            taxable = q2((sp - (sp * rate)) * Decimal(qty))

            sdt = getattr(sale, "transaction_date", None)
            sdt_str = sdt.strftime("%d/%m/%Y") if sdt else ""

            rows.append({
                "sr": i,
                "salesman": salesman_name,
                "customer": loc_obj,            # ✅ outlet name
                "inv": getattr(sale, "invoice_no", "") or "",
                "date": sdt_str,
                "product": prod_name,
                "qty": 1 if qty <= 0 else qty,  # ✅ default 1
                "amount": float(line_amt),
                "taxable": float(taxable),
                "createdBy": loc_obj,           # ✅ outlet name
                "location": loc_obj,            # keep for column
            })
        ...
        payload = {
            ...
            "kpis": {
                "target": 0,
                "total_sales": float(q2(total_sales)),
                "difference": 0,
                "wages": 0,
                "commission": 0,
                "extra_wages": 0,
        ...
        # If paginated
        paginator = _StdReportPagination()
        # build paginator-like response shape
        return Response(payload, status=status.HTTP_200_OK)